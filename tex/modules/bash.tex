...
& \texttt{chmod X$\pm$Y} -- change permissions. \textbf{X} = \texttt{u}:user, \texttt{g}:group, \texttt{o}:other, \texttt{a}:all. \textbf{Y} = \texttt{r}:read, \texttt{w}:write, \texttt{x}:execute.
...
& \texttt{command1 | command2} -- pipe output of command\_1 to input of command\_2.
\end{easylist}
...
& \texttt{CNTRL Z} -- put a foreground process in the background. Use \texttt{\&} at end of command to do this automatically.
...
& \texttt{sed 's/<old>/<new>/g' <stream/file>} -- sed for substitution.
\newline Use \verb!-i'.original'! for in-file replacement. Note you can use different delimiters.
...
& \texttt{.bashrc} OR \texttt{.bash\_profile} -- for permanent aliases and setting of variables.
\end{easylist}
