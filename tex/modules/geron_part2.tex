...
\texttt{test\char`_splt,val\char`_splt,train\char`_splt = tfds.Split.TRAIN.subsplit([10,15,75])}\newline
\texttt{test\char`_set  = tfds.load(\textquotesingle tf\char`_flowers\textquotesingle, split=test\char`_splt,  as\char`_supervised=True)}\newline
\texttt{valid\char`_set = tfds.load(\textquotesingle tf\char`_flowers\textquotesingle, split=val\char`_splt, as\char`_supervised=True)}\newline
\texttt{train\char`_set = tfds.load(\textquotesingle tf\char`_flowers\textquotesingle, split=train\char`_splt, as\char`_supervised=True)}
...
\texttt{test\char`_set = test\char`_set.map(preprocess).batch(32).prefetch(1)}
...
\texttt{output = keras.layers.Dense(5, activation=\textquotesingle softmax\textquotesingle)(avg)}\newline
