...
\vspace{+5.0mm}
\textbf{Probability and Likelihood}
\newline
The terms `probability' and `likelihood' are often used interchangeably in the English language,
but they have very different meanings in statistics...

Given a statistical model with some parameters, \boldmath$\theta$,
the word probability is used to describe low plausible a future outcome, \boldmath$x$, is.
The word likelihood is used to describe how plausible a particular set of parameter values, \boldmath$\theta$,
are after the outcome, \boldmath$x$, is known.

PDF = function of \boldmath$x$, with \boldmath$\theta$ fixed.\newline
Likelihood function = function of \boldmath$\theta$, with \boldmath$x$ fixed.
